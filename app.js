...
function coachStatus(){const c=client(),p=program(),f=diet(),ex=p?(p.weeks||[]).flatMap(w=>w.days.flatMap(d=>d.sections.flatMap(s=>s.exercises))):[],done=ex.filter(x=>c.workoutStatus[x.id]).length,meals=f?.meals||[],md=meals.filter(id=>c.mealStatus[id]).length;return `<div class="card"><h3>Asiakkaan ilmoittama toteuma</h3><div class="grid"><div class="stat">Treeni <b>${done}/${ex.length}</b></div><div class="stat">Ruuat <b>${md}/${meals.length}</b></div></div>${c.alternatives.length?`<div class="note" style="margin-top:9px"><b>Vaihtoehtoiset ruuat</b><br>${c.alternatives.map(a=>{let m=D.meals.find(x=>x.id===a.mealId);return `${esc(m?.name||"Ateria")}: ${esc(a.text)}`}).join("<br>")}</div>`:""}</div>`}
...
function workouts(){
 const c=client();
 return `<h1>Harjoitukset</h1>${clientBar()}<div class="card"><h3>${esc(c.name)}</h3><div class="muted">${esc(program()?.name||"Ei ohjelmaa")}</div></div><button class="btn primary wide" id="newProgram">+ Luo harjoitusohjelma</button><h3 style="margin-top:20px">Treenipankki</h3>${D.library.map(x=>`<div class="card"><div class="row"><div><h3>${esc(x.name)}</h3><div class="muted">${x.exercises.length} liikettä</div></div><button class="btn primary" data-use-library="${x.id}">Asiakkaalle</button></div></div>`).join("")||`<div class="card muted">Treenipankki on tyhjä.</div>`}<h3 style="margin-top:20px">Ohjelmat</h3>${D.programs.map(p=>`<div class="card"><div class="row"><div><h3>${esc(p.name)}</h3><div class="muted">${esc(p.goal)}</div></div><div><button class="btn" data-edit-program="${p.id}">Muokkaa</button><button class="btn danger" data-delete-program="${p.id}">Poista</button></div></div><button class="btn soft wide" data-assign="${p.id}">Aseta asiakkaalle</button></div>`).join("")||`<div class="card muted">Ei ohjelmia.</div>`}`
}
...
function clientHome(){const c=client(),p=program(),f=diet(),ex=p?(p.weeks||[]).flatMap(w=>w.days.flatMap(d=>d.sections.flatMap(s=>s.exercises))):[],done=ex.filter(x=>c.workoutStatus[x.id]).length,meals=f?.meals||[],md=meals.filter(id=>c.mealStatus[id]).length;return `<h1>Oma päivä</h1><div class="card"><h3>${esc(c.name)}</h3><div class="muted">Paino ${c.weight} kg</div></div><div class="grid"><div class="card"><div class="muted">Treeni</div><h2>${done}/${ex.length}</h2><button class="btn primary wide" data-nav="clientworkout">Avaa</button></div><div class="card"><div class="muted">Ruoka</div><h2>${md}/${meals.length}</h2><button class="btn primary wide" data-nav="clientfood">Avaa</button></div></div>`}
function clientWorkout(){const c=client(),p=program();if(!p)return `<h1>Harjoitus</h1><div class="card muted">Ei ohjelmaa.</div>`;return `<h1>Harjoitus</h1>${p.weeks.map((w,wi)=>`<div class="week"><button class="weekToggle" data-client-week="${wi}"><span><b>${esc(w.name||("Viikko "+(wi+1)))}</b><span class="muted"> · ${w.days.length} päivää</span></span><span>${c.openWeeks[wi]===false?"+":"−"}</span></button>${c.openWeeks[wi]===false?"":w.days.map((d,di)=>`<div class="day"><h3>${esc(d.name||("Päivä "+(di+1)))}</h3>${d.sections.map(sec=>`<div class="section"><h3>${esc(sec.name)}</h3>${sec.exercises.map(x=>`<div class="exercise"><div class="row"><div><b>${esc(x.name)}</b><div class="muted">${x.sets} × ${esc(x.reps)} · ${esc(x.rest)}</div></div>${statusPill(!!c.workoutStatus[x.id])}</div><button class="btn ${c.workoutStatus[x.id]?"success":"primary"} wide" data-check-workout="${x.id}">${c.workoutStatus[x.id]?"Merkitse tekemättömäksi":"Kuittaa tehdyksi"}</button></div>`).join("")}</div>`).join("")}</div>`).join("")}</div>`).join("")}`}
function clientFood(){const c=client(),f=diet();if(!f)return `<h1>Ruoka</h1><div class="card muted">Ei ruokavaliota.</div>`;const planned=(f.meals||[]).reduce((a,id)=>a+(D.meals.find(x=>x.id===id)?.kcal||0),0),eaten=(f.meals||[]).filter(id=>c.mealStatus[id]).reduce((a,id)=>a+(D.meals.find(x=>x.id===id)?.kcal||0),0);return `<h1>Ruoka</h1><div class="card"><div class="grid"><div class="stat"><div class="muted">Suunnitellut</div><b>${planned} kcal</b></div><div class="stat"><div class="muted">Syödyt</div><b>${eaten} kcal</b></div></div><div class="badges" style="margin-top:9px"><span class="badge">${f.kcal} kcal tavoite</span><span class="badge">P ${f.p} g</span><span class="badge">H ${f.c} g</span><span class="badge">R ${f.f} g</span></div></div>${(f.meals||[]).map(id=>{let m=D.meals.find(x=>x.id===id),alt=c.alternatives.find(a=>a.mealId===id);return m?`<div class="meal"><div class="row"><div><h3>${esc(m.name)}</h3><div class="muted">${m.kcal} kcal · P ${m.p} · H ${m.c} · R ${m.f}</div></div>${statusPill(!!c.mealStatus[id],!!alt)}</div><button class="btn ${c.mealStatus[id]?"success":"primary"} wide" data-check-meal="${id}">${c.mealStatus[id]?"Merkitse syömättömäksi":"Kuittaa syödyksi"}</button><button class="btn soft wide" data-alt-meal="${id}">Ilmoita vaihtoehtoinen ruoka</button>${alt?`<div class="note">Vaihtoehto: ${esc(alt.text)}</div>`:""}</div>`:""}).join("")}`}
...
function bind(){
 ...
 const np=$("#newProgram");if(np)np.onclick=()=>openProgram({id:Date.now(),name:"Uusi ohjelma",goal:"Voima",weeks:[{name:"Viikko 1",days:[{name:"Treeni A",sections:[{name:"Pääosio",exercises:[]}]}]}]});
 ...
 document.querySelectorAll("[data-use-library]").forEach(b=>b.onclick=()=>{const l=D.library.find(x=>x.id===+b.dataset.useLibrary);const p={id:Date.now(),name:l.name,goal:l.goal,weeks:[{name:"Viikko 1",days:[{name:"Treeni A",exercises:JSON.parse(JSON.stringify(l.exercises))}]}]};D.programs.push(p);client().programId=p.id;save();render()});
...
function openProgram(p){
 ...
 <div class="actions"><button class="btn primary wide" id="savep">Tallenna asiakkaalle</button><button class="btn primary wide" id="savelib">Tallenna myös treenipankkiin</button></div></div></div>`);
 const m=document.querySelector(".modal:last-child");
